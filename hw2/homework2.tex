...
\begin{align*}
 ...
 \newpage 
 \textbf{1.} To find the log likelihood, we can start with the equation for likelihood and work 
 from there. The equation for likelihood of one point $(x_i, y_i)$ is expressed as $P(x_i,y_i | \pi) = P(x_i | y_i = C_k)P(y_i = C_k | \pi)$ 
 Therefore across the whole dataset it would result in $P(D|\pi) = \Pi_{i=1}^N[P(x_i | y_i = C_k)P(y_i = C_k | \pi)]$. Then we also know that the 
 prior which is that $P(y = C_k | \pi) = \pi_k$, so we know $P(D|\pi) = \Pi_{i=1}^N[P(x_i | y_i = C_k)\pi_k]$. Then, we can take the log of each side 
 to get $\ln(P(D|\pi)) = \sum_{i=1}^{N}ln[P(x_i|y_i = C_k) * \pi_k]$ \\\\

 \textbf{2.} As per the hint provided in the problem, we know that $L(x, \lambda) = f(x) + \lambda g(x)$ and we know in this case that $f(x)$ is the log likelihood,
 and that $g(x)$ is the restriction on $\pi_k$, so we have $L(\pi, \lambda) = \ln(P(D | \pi)) + \lambda(\sum_{k}\pi_k-1)$
